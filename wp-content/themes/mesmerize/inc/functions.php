...
function mesmerize_can_show_cached_value($slug)
{
    ...
    $result = (intval(get_option("mesmerize_has_cached_{$slug}", false)) !== 0);
...
function mesmerize_cache_value($slug, $value, $autoload = 'yes', $cache_on_ajax = false)
{
    ...
    update_option($slug, $value, $autoload);
    update_option("mesmerize_has_cached_{$slug}", true, $autoload);
    ...
    if ( ! in_array($slug, $slugs)) {
        ...
        $slugs[] = $slug;
        update_option("mesmerize_cached_values_slugs", $slugs, $autoload);
    }
}
...
{
    delete_option($slug);
    delete_option("mesmerize_has_cached_{$slug}");
}

function mesmerize_get_cached_value($slug, $default = false)
{
    if (mesmerize_can_show_cached_value($slug)) {
